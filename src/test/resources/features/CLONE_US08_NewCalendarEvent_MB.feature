...
Feature: New calendar event
  ...
  Scenario: User should see an error message “This value should not be blank.” when the Calendar event repeat field is cleared
    ...
    Scenario: User is able to create a recurring event
      And user fills out Title "test title Maryna" on Create Calendar Event page
      And user is able to write "test Maryna" in description input field
      ...
      And user clicks on "repeat" checkbox
      And user chooses Repeats "Daily"
      ...
      And the user navigates to "Activities" tab and "Calendar Events" module
      And user sees new event on Calendar Events page with "test title Maryna" title
